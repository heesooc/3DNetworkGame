...
public class Bear : MonoBehaviour, IPunObservable
{
    ...
    private void Idle()
    {
        ...
        {
            _idleTime = 0f;
            SetRandomPatrolDestination(); // PatrolPoint 선정
    ...
    private void Return()
    {
        ...
        if (!Agent.pathPending && Agent.remainingDistance <= 0.1f) // Agent.pathPending: 경로 계산이 아직 진행 중
        {
    ...
    private void Trace()
    {
        ...
        // 타겟이 죽거나 너무 멀어지면 복귀
        Agent.destination = _targetCharacter.transform.position; // 이동 목적지: Trace
